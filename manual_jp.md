...
#### ① `mppo_ai/clients/rl_bot.py` (強化学習AIクライアント)
サーバーに学習済みAIを接続させます。
```bash
python mppo_ai/clients/rl_bot.py --model mppo_ai/rl/output/fafnir_final.zip [オプション]
```
| 引数 | デフォルト | 説明 |
| :--- | :--- | :--- |
| `--model` | (必須) | 使用するモデルのパス。 |
...
| `--det-batch` | `8` | 1回のGPU評価にまとめる決定化パターン数。大きいほどGPU効率が上がりますが、時間制限の粒度が粗くなります。 |
