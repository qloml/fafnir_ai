...
def setup_offer(game: Game) -> bool:
    ...
    stones: List[str] = [] # 袋から引く石リストの宣言
    # random.shuffle(game.bag) # 高速化のため不要なシャッフルを削除
    ...
    while True:
        ...
        # 色が複数になった，または袋が空になったら確定
        if len(set(stones)) > 1 or len(game.bag) == 0:
            break

    # ループ中に引いたすべての石がofferとなる
    game.offer = stones
...
def log_hands(game: Game, title: str = ""):
    if title:
        logger.debug(title)
    for i, p in enumerate(game.players):
        logger.debug(f"  HAND P{i} {p.name}: {_summ_counts(p.stones)}")
        logger.debug(f"    list: {p.stones}")
...
def log_auction_result(game: Game):
    ...
    logger.debug("┌────────────────────────────── AUCTION RESULT ──────────────────────────────┐")
    logger.debug(f"│ Round={game.round} Turn={game.turn}  Caretaker={game.caretaker}  Offer={offer}")
    logger.debug(f"│ bids_count={bids_count}")
    for i, p in enumerate(game.players):
        b = _get_bids_from_last_result(lr, i)
        logger.debug(f"│  P{i} {p.name:<10} bid: {_summ_counts(b)}  | list={b}")
    logger.debug(f"│ winner={w} loser={l}  used_by_winner={_summ_counts(used)}  | list={used}")
    logger.debug(f"│ score: P0={game.players[0].score}  P1={game.players[1].score}")
    logger.debug(f"│ trash: {game.trash}   (limit={TRASH_LIMIT})")
    logger.debug("└────────────────────────────────────────────────────────────────────────────┘")
...
def check_game_end(game: Game) -> Optional[Dict[str, Any]]:
    for i, p in enumerate(game.players):
        if p.score >= SCORE_TO_WIN:
            return {"reason": "SCORE_40", "winner": i, "score_to_win": SCORE_TO_WIN}
    return None
...
class Room:
    def __init__(self, room_id: str):
        ...
        self.wins_by_player: Dict[str, int] = {}
        self.draws: int = 0
...
def start_game_if_ready(room: Room):
    ...
    logger.debug(f"[{ts()}][SERVER] GAME START room={room.room_id} caretaker={game.caretaker} offer={game.offer}")
...
def start_new_game(room: Room):
    ...
    logger.debug(f"[{ts()}][SERVER] NEW GAME room={room.room_id} caretaker={game.caretaker} offer={game.offer}")
...
def build_state(room: Room, viewer_index: int, viewer_mode: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "round_end_info": game.round_end_info,
        "game_end_info": game.game_end_info,
        "connections": connections,
        "action_log": room.action_log[-80:],
        "ts": time.time(),
    }
...
def start_round_end(game: Game, trigger_reason: str, trigger_color: Optional[str], last_bid_winner: Optional[int]):
    ...
    logger.debug(f"[{ts()}][SERVER] ROUND_END reason={trigger_reason} trigger_color={trigger_color} rw={rw} caretaker={game.caretaker}")
...
async def connect(sid, environ, auth):
    logger.debug(f"[{ts()}][SERVER] connect sid={sid}")
    await sio.emit("server_info", {"msg": "connected"}, to=sid)


@sio.event
async def disconnect(sid):
    logger.debug(f"[{ts()}][SERVER] disconnect sid={sid}")
    for room_id, room in list(rooms.items()):
        if sid in room.sids or sid in room.spectators:
            ...
            logger.debug(f"[{ts()}][SERVER] {name} left room={room_id} role={role} after {dur}s")
...
async def join_room(sid, data: Dict[str, Any]):
    ...
    if room.is_full() and sid not in room.sids:
        ...
        logger.debug(f"[{ts()}][SERVER] join denied sid={sid} room={room_id} (full)")
        return
    ...
    logger.debug(f"[{ts()}][SERVER] join_room sid={sid} room={room_id} idx={idx_now} name={name} sids={len(room.sids)}")
...
async def submit_bid(sid, data: Dict[str, Any]):
    ...
    logger.debug(f"[{ts()}][SERVER] submit_bid from P{player_index} {p.name} -> bid={filtered}")
    await emit_action(
        room,
        "submit_bid",
        {
    ...
    if len(game.players) == 2 and all(pp.bid_submitted for pp in game.players):
        ...
        if end:
            w_idx = end.get('winner')
            if w_idx is not None:
                p_name = game.players[w_idx].name
                room.wins_by_player[p_name] = room.wins_by_player.get(p_name, 0) + 1
            else:
                room.draws += 1
            room.match_count += 1
            
            logger.warning(f"--- MATCH {room.match_count}/{TARGET_MATCHES} END ---")
            logger.warning(f"Current Wins: {room.wins_by_player}, Draws: {room.draws}")
            ...
            if room.match_count >= TARGET_MATCHES:
                logger.warning(f"========== COMPARE FINISHED ==========")
                logger.warning(f"Total Matches: {room.match_count}")
                for name, wins in room.wins_by_player.items():
