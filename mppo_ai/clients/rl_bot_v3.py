...
def state_to_obs(st: Dict[str, Any], player_idx: int, score_to_win: int = 40) -> np.ndarray:
    """Convert server state_update dict to 34-dim observation vector.
    ...
    # 31: bag remaining
    obs[31] = st.get("bag_left", 0) / float(max(1, sum(INITIAL_BAG)))

    # 32: am I caretaker
    obs[32] = 1.0 if st.get("caretaker") == player_idx else 0.0
